...
def find_experimental_data(vem, tram):
    ...
    for exp in TRAINING_DATA_LIST:
        d = exp['drugs']
        if abs(d['vemurafenib'] - vem) < 1e-4 and           abs(d['trametinib'] - tram) < 1e-4:
            return exp
    return None
...
def plot_key_species_summary(df):
    ...
    for sp_idx, species in enumerate(KEY_SPECIES):
        ...
        for exp in TRAINING_DATA_LIST:
            d = exp['drugs']
            if abs(d.get('pi3k_inhibitor', 0.0) - 0.0) < 1e-4 and               abs(d.get('ras_inhibitor', 0.0) - 0.0) < 1e-4:
                if exp['name'] == 'Vemurafenib Only (0.5)':
                    ax.scatter(exp['time_points'], exp['species'][species],
                    ...
                    if any(abs(d['vemurafenib'] - v) < 1e-4 for v in VEM_DOSES) and                       any(abs(d['trametinib'] - t) < 1e-4 for t in TRAM_DOSES):
