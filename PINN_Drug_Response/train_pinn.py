...
from physics_utils import compute_physics_loss, compute_conservation_loss, compute_steady_state_loss
...
def train_pinn(config: TrainingConfig, condition_name: Optional[str] = None) -> Tuple[nn.Module, nn.ParameterDict, List[Dict], Dict, Dict, Dict]:
    ...
    param_defaults = {
        ...
        'w_egfr': 1.0, 'w_her2': 1.0, 'w_her3': 1.2, 'w_igf1r': 1.0,
        'w_craf': 1.2, 'w_mek': 1.8, 'w_erk': 2.5, 'w_dusp6': 1.5,
        'w_akt': 2.5, 'w_4ebp1': 1.3
    }
    ...
    for epoch in progress_bar:
        ...
        total_loss = (config.weights.data * l_data +
                      ...
                      config.weights.steady_state * l_steady_state)
        ...
        if epoch % 100 == 0:
            ...
            logger.info(f"Epoch {epoch}: Total={total_loss.item():.2e} | Data={l_data.item():.2e} | Phys={l_physics.item():.2e} | Bound={l_boundary.item():.2e} | SS={l_steady_state.item():.2e} | Cons={l_conservation.item():.2e} | Test={l_test:.2e}")
            progress_bar.set_postfix({'loss': f"{total_loss.item():.2e}", 'data': f"{l_data.item():.2e}", 'SS': f"{l_steady_state.item():.2e}"})
            checkpoint_metric = l_test if (has_test_data and t_test is not None and l_test > 0) else l_data.item()
    ...
    final_save_path = f"{config.output_dir}/pinn_model_{condition_name.replace(' ', '_') if condition_name else 'global'}_final.pth"
    save_checkpoint(checkpoint_data, final_save_path, logger=None)
