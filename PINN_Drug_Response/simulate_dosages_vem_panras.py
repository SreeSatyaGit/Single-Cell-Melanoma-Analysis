"""
Drug Dosage Simulation: Vemurafenib + panRAS Inhibitor Combinations
===================================================================
Simulates pathway trajectories across a grid of Vem/RAS inhibitor dosages
...
OUTPUT_DIR = 'analysis_results/dosage_simulation_vem_panras'
VEM_DOSES = [0.0, 0.1, 0.25, 0.5, 1.0, 2.0]
RAS_DOSES = [0.0, 0.1, 0.25, 0.5, 1.0]
T_MAX = 80
T_POINTS = 300
...
def simulate_all_combinations(model, scalers, device):
    ...
    all_results = []
    for vem, ras in product(VEM_DOSES, RAS_DOSES):
        drugs = {
            'vemurafenib': vem,
            'trametinib': 0.0,
            'pi3k_inhibitor': 0.0,
...
def plot_species_trajectories(df, t_sim):
    ...
    for species in SPECIES_ORDER:
        ...
        for ax_idx, ras in enumerate(RAS_DOSES):
            ...
            for v_idx, vem in enumerate(VEM_DOSES):
                ...
                if exp:
                    ...
                    elif exp['name'] == 'Vem + panRAS Combo':
                        ax.scatter(exp['time_points'], exp['species'][species],
                                   color='greenyellow', s=60, edgecolors='black',
                                   linewidths=1.2, zorder=12, label='Exp: Vem+RAS')
                    else:
                        ax.scatter(exp['time_points'], exp['species'][species],
                                   color=colors[v_idx], s=45, edgecolors='white',
                                   linewidths=1.2, zorder=10, alpha=0.8)

            ax.set_title(f'panRAS = {ras} µM', fontsize=11, fontweight='bold')
        ...
        fig.suptitle(f'{species} — Vem + panRAS Dosage Simulation', fontsize=14, fontweight='bold')
...
def plot_key_species_summary(df):
    combos = list(product(VEM_DOSES, RAS_DOSES))
    total_doses = [v + r for v, r in combos]
    ...
    for sp_idx, species in enumerate(KEY_SPECIES):
        ax = axes[sp_idx]
        for vem, ras in combos:
            mask = (df['vemurafenib'] == vem) & (df['ras_inhibitor'] == ras)
            subset = df[mask].sort_values('time')
            color = cmap(norm(vem + ras))
            ...
            if exp is not None:
                if exp['name'] == 'Vemurafenib Only (0.5)':
                    ...
                    ax.scatter(exp['time_points'], exp['species'][species],
                               color='greenyellow', s=65, edgecolors='black',
                               linewidths=1.2, zorder=15, label='Experimental: Vem+RAS')
                else:
                    ax.scatter(exp['time_points'], exp['species'][species],
    ...
    cbar.set_label('Total Drug Load (Vem + panRAS µM)', fontsize=11)
    fig.suptitle('Key Species Response — Vem + panRAS Dosage Sweep', fontsize=16, fontweight='bold')
...
def plot_dose_response_heatmaps(df):
    ...
    for sp_idx, species in enumerate(KEY_SPECIES):
        for t_idx, t_snap in enumerate(HEATMAP_TIMEPOINTS):
            ...
            heatmap = np.zeros((len(RAS_DOSES), len(VEM_DOSES)))
            for v_i, vem in enumerate(VEM_DOSES):
                for r_i, ras in enumerate(RAS_DOSES):
                    row = snapshot[(snapshot['vemurafenib'] == vem) & (snapshot['ras_inhibitor'] == ras)]
                    if len(row) > 0:
                        heatmap[r_i, v_i] = row[species].values[0]
            im = ax.imshow(heatmap, aspect='auto', origin='lower', cmap='viridis',
                          extent=[min(VEM_DOSES), max(VEM_DOSES), min(RAS_DOSES), max(RAS_DOSES)])
            if sp_idx == 0:
                ax.set_title(f't = {t_snap}h', fontsize=11, fontweight='bold')
            if t_idx == 0:
                ax.set_ylabel(f'{species}\npanRAS (µM)', fontsize=9)
            else:
                ax.set_ylabel('')
            if sp_idx == len(KEY_SPECIES) - 1:
                ax.set_xlabel('Vem (µM)', fontsize=9)
            plt.colorbar(im, ax=ax, shrink=0.8)
    fig.suptitle('Dose-Response Heatmaps — Vem × panRAS Grid', fontsize=16, fontweight='bold')
...
def main():
    ...
    print("=" * 60)
    print("PINN Drug Dosage Simulation: Vemurafenib + panRAS Inhibitor")
    print("=" * 60)
    print(f"Vem doses:    {VEM_DOSES}")
    print(f"panRAS doses: {RAS_DOSES}")
    print(f"Total combinations: {len(VEM_DOSES) * len(RAS_DOSES)}")
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'simulation_vem_panras_all.csv')
